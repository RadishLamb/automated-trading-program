  ...
  if (store$iter > 33) {
    ...
    #    pos2[params$series[i]] <- lgStFtkdj(store$cl,store$high,store$low,store$index,params$series[i],store$iter)*params$posSizes[params$series[i]]
     ...
     # }
      #if (i==6){
        #CCI
      ppos[i]<-lgStFtkdj(store$cl,store$high,store$low,store$index,params$series[i],store$iter,store$ppooss)
...
lgStFtkdj <-	function(clStore,highStore,lowStore,indexStore,column,iter,ppooss) {
  ...
  ndate<-length(date)
  date1<-indexStore[1:iter,column]
  ndate1<-length(date1)
  #print(ndate)
  ...
  posP<-ppooss[1:iter,column]
  close<-xts(clStore[1:iter,column],order.by = as.Date(date1[1:ndate1]))
  if(posP[iter-1]==0){
    long<-ifelse(KDSignal >= 1,1,0)
    short<-ifelse(KDSignal <= -1,-1,0)
    poss<-long+short
    if(poss!=posP[iter-1]) entry<<-iter
  } else {
    rett<-getTradeReturn(close,entry,exit = iter,isTRUE(posP[entry]<0))
