...
#      print(store$cl[store$iter,i])
     # if(store$cl[store$iter,i]<StopLossUp && store$cl[store$iter,i]>StopLossDn)
      #{
        ppos[i] <- lgStFt(store$cl,params$series[i],store$iter,store$vo)
...
lgStFt <-	function(clStore,column,iter,voStore) {
	...
	fastVwap<-VWAP(clStore[startIndex:iter,column],voStore[startIndex:iter,column],6)
	lastfastVwap<-last(fastVwap)
	slowVwap<-VWAP(clStore[startIndex:iter,column],voStore[startIndex:iter,column],36)
	lastslowVwap<-last(slowVwap)

  
  #print(isTRUE(kst>signal))
	longs<-ifelse(lastfastVwap>lastslowVwap,1,0)
	shorts<-ifelse(lastfastVwap<lastslowVwap,-1,0)
	#print(longs)
	#print(shorts)
	return(longs+shorts)
