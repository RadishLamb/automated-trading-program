...
getOrders <- function(store, newRowList, currentPos, params) {
    ...
    pos1<-rep(0,length(params$series))
    if (store$iter > 27) {
...
calculatePosSizes<-function(clStore,highStore,lowStore,column,iter){
  startIndex <- iter - 27 - 1
  
  closeP<-clStore[1:iter,column]
  highP<-highStore[1:iter,column]
  lowP<-lowStore[1:iter,column]
...
lgStFtcci <-	function(clStore,highStore,lowStore,indexStore,column,iter,ppooss,entryAndExit) {
 # print(entryAndExit)
  startIndex <- iter - params$lookbackS - 1
