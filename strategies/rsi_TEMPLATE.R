...
getOrders <- function(store, newRowList, currentPos, params) {
	...
	if (store$iter > params$lookbackS) {
...
checkParams <- function(params) { # make sure params are correct
    if (!"lookbackS" %in% names(params))
        stop("Parameter lookbackS not defined for strategy RSI")
    if (!"lookbackL" %in% names(params))
        stop("Parameter lookbackL not defined for strategy RSI")
    if (!"threshold" %in% names(params))
        stop("Parameter threshold not defined for strategy RSI")
    if (params$threshold < 0 || params$threshold > 50)
        stop("Parameter lookbackS threshold is not between 0 and 50")
}
